...
def main():
    ...
    try:
        ...
        try:
            ...
            for name in ("pygame.sysfont", "pygame.font"):
                module = sys.modules.get(name)
                if module is not None and not hasattr(module, "Font"):
                    del sys.modules[name]

            font_module = importlib.import_module("pygame.font")
            font_module.init()
            try:
                font = font_module.SysFont("Courier", 16)
            except Exception:
                font = font_module.Font(None, 16)
        except Exception as exc:
            print(f"Warning: Could not load pygame.font fallback ({exc})")
